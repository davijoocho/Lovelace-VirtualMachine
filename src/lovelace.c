...
int main(int argc, char* argv[])
{
    ...
    mf_i = (mf_i | metadata[3]) << 24 | (mf_i | metadata[2]) << 16 | (mf_i | metadata[1]) << 8 | (mf_i | metadata[0]);
    cp_i = (cp_i | metadata[7]) << 24 | (cp_i | metadata[6]) << 16 | (cp_i | metadata[5]) << 8 | (cp_i | metadata[4]);
